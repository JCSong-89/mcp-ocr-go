...
def image_ocr_to_text(image_paths):
    total_trust_point = 0
    get_text_count = 0
    ocr_text = []
...
# gpu=False 옵션은 GPU가 없거나 VRAM이 부족할 경우 CPU를 사용하도록 합니다.
    ...
    print(f'"총 점수: {total_trust_point} \ 총 텍스트: {get_text_count}')
    total_trust_point = total_trust_point / get_text_count
    print(f"총합 신뢰도: {total_trust_point}")
    print(f"종합 데이터: \n {ocr_text}")
